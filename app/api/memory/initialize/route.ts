...
export async function POST(request: NextRequest) {
  try {
    ...
    const service = new MemoryServiceV4({
      ...
      orgId: undefined // Will use default from the service
    });

    // Initialize all compartments with their custom instructions
    await service.initializeProject();

    return NextResponse.json({
      success: true,
      message: 'Mem0 compartments initialized successfully',
      projectId: projectId
    });
  } catch (error) {
